#!/usr/bin/env python3
"""
BrainBit Filtered Visualization

This script shows only filtered EEG signals with each channel independently
normalized to better assess signal quality.
"""
...
class BrainBitHandler:
    ...
    def normalize_signal(self, data):
        """Normalize signal to range [-1, 1] if it has non-zero variance."""
        if np.var(data) > 0:
    ...
    def update_data(self):
        ...
        global eeg_data_filtered, should_run
            ...
            try:
                ...
                with data_lock:
                    ...
                    for idx, ch_idx in enumerate(eeg_channels):
                        ...
                        eeg_data_filtered[ch_name] = self.normalize_signal(filtered)
...
def init_plot(fig, axes_eeg, axes_psd, lines_eeg, lines_psd, lines_fit):
    """Initialize the plot."""
    ...
    for i, (ch_name, ch_data) in enumerate(eeg_data_filtered.items()):
        ...
        axes_eeg[i].set_ylim(-1.1, 1.1)  # Normalized range
    ...
    return lines_eeg + lines_psd + [line for line in lines_fit if line]

def update_plot(frame, handler, fig, axes_eeg, axes_psd, lines_eeg, lines_psd, 
               lines_fit, alpha_texts, status_text):
    ...
    with data_lock:
        ...
        for i, (ch_name, ch_data) in enumerate(eeg_data_filtered.items()):
            # Update normalized signal (always in range [-1, 1])
            lines_eeg[i].set_data(x_data, ch_data)
            
            # Compute and update PSD
            f, psd = handler.compute_psd(ch_data, handler.sample_rate)
...
def create_filtered_visualization():
    """Create visualization with filtered, normalized EEG signals and spectral analysis."""
    # Connect to BrainBit
    ...
    for i, ch_name in enumerate(channel_names):
        ...
        ax.set_title(f"Channel {ch_name} (Normalized)")
        ax.set_ylabel('Amplitude')
        ax.set_xlabel('Time (s)' if i == len(channel_names) - 1 else '')
        ax.grid(True, alpha=0.3)
        
        # Add signal quality indicator
        ax.text(0.02, 0.95, "Normalized Signal", transform=ax.transAxes, fontsize=9,
    ...
    # Set figure title
    fig.suptitle('BrainBit Filtered & Normalized EEG', fontsize=16)
    
    # Create animation
    ani = FuncAnimation(
        fig, update_plot, 
        fargs=(handler, fig, axes_eeg, axes_psd, lines_eeg, 
