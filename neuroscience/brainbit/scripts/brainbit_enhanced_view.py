...
# Fixed y-axis limits
eeg_y_limit = 50000      # ±50,000 μV for raw EEG
power_y_limit = 500      # 0-500 μV²/Hz for band power
spectral_y_limit = 1e6   # 0-1,000,000 for PSD
...
def show_current_tab():
    ...
    for ax in eeg_axes:
        ax.set_visible(current_tab == "raw")
    
    for ax in power_axes:
        ax.set_visible(current_tab == "power")
    
    for ax in spectral_axes:
        ax.set_visible(current_tab == "spectral")
    
    fig.canvas.draw_idle()

def main():
    """Main function to connect to BrainBit and display data."""
    global board, fig, gs, eeg_axes, power_axes, spectral_axes
    global eeg_lines, power_bars, psd_lines, fit_lines, current_tab
    ...
    for i, ch_name in enumerate(channel_names):
        ...
        # Set up axes
        ax.set_title(f"Channel {ch_name} - Spectral Analysis", fontsize=12)
