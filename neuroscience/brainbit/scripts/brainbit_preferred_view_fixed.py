...
def apply_filters(data, sample_rate):
    ...
    try:
        ...
        # 2. Apply notch filter to remove 60Hz line noise (using appropriate function call)
        # Note: The environmental_noise filter takes different parameters in different versions
        try:
            # Try current API (4 args)
            DataFilter.remove_environmental_noise(filtered_data, sample_rate, 50, 60)
        except Exception:
            try:
                # Try older API (3 args)
                DataFilter.remove_environmental_noise(filtered_data, sample_rate, 60)
            except Exception as e:
                print(f"Notch filter error: {e}, skipping")
